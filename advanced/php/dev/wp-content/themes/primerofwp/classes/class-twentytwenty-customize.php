...
if ( ! class_exists( 'twentytwenty_Customize' ) ) {
	...
	class twentytwenty_Customize {
		...
		public static function register( $wp_customize ) {
			...
			/* Enable Home Recent Card ---------------------------------------------------- */

			$wp_customize->add_setting(
				'enable_home_recent_card',
				array(
					'capability'        => 'edit_theme_options',
					'default'           => true,
					'sanitize_callback' => array( __CLASS__, 'sanitize_checkbox' ),
			...
			$wp_customize->add_control(
				...
				array(
					...
					'section'  => 'options',
					'priority' => 10,
					'label'    => __( 'トップページに新着記事を表示（カード型）', 'twentytwenty' ),
				)
			);

			/* Enable Home Recent List ---------------------------------------------------- */

			$wp_customize->add_setting(
				'enable_home_recent_list',
				array(
					'capability'        => 'edit_theme_options',
					'default'           => true,
					'sanitize_callback' => array( __CLASS__, 'sanitize_checkbox' ),
			...
			$wp_customize->add_control(
				...
				array(
					...
					'section'  => 'options',
					'priority' => 10,
					'label'    => __( 'トップページに新着記事を表示（リスト型）', 'twentytwenty' ),
				)
			);
...
*/
			...
			$wp_customize->add_control(
				...
				array(
					...
					'section'  => 'options',
					'priority' => 10,
					'label'    => __( '投稿一覧ページの表示', 'twentytwenty' ),
					'choices'  => array(
						'full'    => __( '全文表示', 'twentytwenty' ),
						'card' => __( 'カード型', 'twentytwenty' ),
						'list' => __( 'リスト型', 'twentytwenty' ),
					),
				)
			);
